...
'Inverse Kinematics of Null Space'
...
if __name__ == "__main__":
    ...
    target = targets[3] ###pick your target here
    ...
    config_up_limit_list = [2.13539289e+00, 1.29629967e+00, 3.74999698e+00, -1.50000054e-01, 1.00000000e+04,
                            -1.00000036e-01, 1.00000000e+04]

    config_down_limit_list = [-5.64601796e-01, -3.53600216e-01, -6.50000756e-01, -2.12130808e+00, -1.00000000e+04,
                              -2.00000770e+00, -1.00000000e+04]
    config_mid_list = []

    for i in range(0,7):
        config_mid_list.append((config_up_limit_list[i]+config_down_limit_list[i])/2.0)
    ...
    while searching>0:
        ...
        if linalg.norm(dta_xyz_ary) < threshold:
            ...
            print 'Found !!! '
            print 'ans_config_ary_nullspace = ', ans_config_ary
            print 'alpha =',alpha
            print 'beta =', beta
        ...
        dta_xyz_mat_t = matrix(dta_xyz_ary).T
        q_increment_mat = psd_ivs_j_mat*dta_xyz_mat_t
        q_increment_mat_t = q_increment_mat.T
        q_increment_lst = q_increment_mat_t.tolist()
        q_two_dta_ary = array(config_mid_list) - q_config_ary
        q_two_dta_ary_T = matrix(q_two_dta_ary).T
        beta_part_T = (((eye(7) - psd_ivs_j_mat*matrix(j_list))*q_two_dta_ary_T).T)

        beta_part_lst = beta_part_T.tolist()

        beta_part_config_ary = array(beta_part_lst[0])

        beta_config_ary = beta*beta_part_config_ary

        q_increment_ary = array(q_increment_lst[0]) + beta_config_ary
