...
def main():
    ...
    plt.plot(range(len(true_locations)),filtered_locations,'g*',label = 'filtered location')
    plt.xlabel('time')
    plt.ylabel('position')
    # plt.legend(('filtered position','sensored error'),loc='upper right')
    plt.title('Kalman Filter with 3 State Varibles')
    ...
    # plt.plot(range(0,distance_f.shape[0]),list(distance_f),'r.-',range(0,distance_s.shape[0]),list(distance_s),'g.-',range(0,distance_f_1.shape[0]),list(distance_f_1),'b.-')
    plt.xlabel('iterations')
    plt.ylabel('error(distance)')
    plt.legend(('filter distance error','sensor distance error'),loc='upper right')
    plt.title('Kalman Filter with 3 State Varibles')
    # plt.legend(('filter distance error','sensor distance error','filter_1 distance error'),loc='upper right')
    plt.show()
