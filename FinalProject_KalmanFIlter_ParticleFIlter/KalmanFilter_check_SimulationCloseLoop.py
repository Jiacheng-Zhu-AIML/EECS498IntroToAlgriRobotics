...
Kalman Filter with Gaussian Sensor and Collision checker
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        if not pr2_motion.working:
            break
        if not pr2_motion.moving:
            continue

        true_position = pr2_motion.TrueLocation()
        true_location_line_list = UpdateTrajectoryLine(env, true_location_line_list, true_location_list[-1], true_position, 0,0,0,10)
        ...
        filter_location_line_list = UpdateTrajectoryLine(env, filter_location_line_list, filtered_location_list[-1], filtered_location, 0,0,1,10)
        ...
        filtered_location_paint_list.extend(PaintPositionBox(env, filtered_location[0], filtered_location[1],0,0,0,1))
