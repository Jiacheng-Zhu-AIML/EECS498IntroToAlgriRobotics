...
Open loop control, A set of motions is given to the robot
...
if __name__ == "__main__":
    ...
    env.Reset()
    env.Load('map_folder/PR2_Maze_Sonar_test.env.xml')
    x_range = (-8, 8)
    y_range = (-24,24)
    time.sleep(0.1)
    ...
    pr2_particles.InitializeParticelsCollision(x_range, y_range, 200, 3)
    ...
    '''
    close_loop_plan = [[6, 0],[-2, 0]]#, [-1, -2], [0, 0], [3, -2], [4, 2]]
    open_loop_plan = [[[0, 1], 10, 0.5]] #, [[1, 0], 10, 0.2], [[1, 1], 40, 0.2]]
    ...
    while pr2_motion.working:
        ...
        pr2_particles.Resample(GaussianResample, 0.70)
