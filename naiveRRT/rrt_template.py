...
from RRT_Path_Finding import RRTPathFindingFunc
from RRT_Path_Finding import *
#### END OF YOUR IMPORTS ####
...
if __name__ == "__main__":
    ...
    result = RRTPathFindingFunc( startconfig, goalconfig , env, 0.05, 0.18)

    path = result[0]
    red_point_list = DrawPath(path, env, 1, 0, 0, 5)
    path2 = path
    print 'origin path=', path

    smoothed = PathSmoothAdvanced(path2,0.02, env)
    print 'parameter path after smooth:'
    print smoothed

    #blue_point_list = DrawPath(smoothed,env,0,0,1,5)
    ...
    print 'path.extend(smoothed)',path
